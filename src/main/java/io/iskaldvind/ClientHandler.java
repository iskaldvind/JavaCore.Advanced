...
class ClientHandler {
    ...
    Client client;
    final long AUTHTIMEOUT = 120000L;
    Timer timer;

    class AuthTimeoutTask extends TimerTask {
        @Override
        public void run() {
            try {
                dataOutputStream.writeUTF("/stop Превышено время ожидания входа");
            } catch (IOException e) {
                e.printStackTrace();
            } finally {
                try {
    ...
    ClientHandler(AuthService authService, Server server, Socket socket) {
        ...
        try {
            ...
            timer = new Timer();
            timer.schedule(new AuthTimeoutTask(), AUTHTIMEOUT);
        ...
        while (true) {
            ...
            if (messageData.length == 3 && messageData[0].equals("/auth")) {
                ...
                if (client != null) {
                    ...
                    dataOutputStream.writeUTF("/err Неправильные логин и пароль!");
                }
            } else {
                dataOutputStream.writeUTF("/err Ошибка авторизации!");
            }
            if (tryCount == maxTryCount) {
                dataOutputStream.writeUTF("/stop Первышен лимит попыток!");
                dataInputStream.close();
                dataOutputStream.close();
                socket.close();
                return false;
